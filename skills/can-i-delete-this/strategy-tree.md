...
1. **Tests added in the same commit.** Check `co_changed` for anything
   `noise.is_test_path` would recognize (a `tests/` directory, a
   `test_`/`_test`/`_spec` filename, a `.test.`/`.spec.` segment, or a
   directory or filename stem ending in the capitalized word `Test`, the
   Android/JVM convention -- e.g. `androidTest/`, `FooTest.kt`; a directory
   match sweeps every path beneath it, not just that one entry). A test

...
against `gitq.has_splitlines_divergence`, the pure predicate the fix
...
out already tests completely. The predicate moved from `trace.py` to
`gitq.py` in the fourth round, once `scan.py` needed the identical check
against the identical hazard (see `tests/test_scan_line_break_divergence.py`)
and duplicating it was worse than sharing it from where `run_git`'s two
read paths already live.
...
# The eight characters `gitq._SPLITLINES_ONLY_BREAKS` names, plus the
# ninth case (a lone "\r") handled separately by `gitq._LONE_CR`. Listed
# here by name, not just imported as the constant, so a change to the
# constant that silently drops one of the nine is caught by this file
# rather than by both sides agreeing with each other.
_VERTICAL_TAB = "\x0b"
_FORM_FEED = "\x0c"
_FILE_SEPARATOR = "\x1c"
...
def _working_tree_line_count(text):
    ...
    checks `gitq.has_splitlines_divergence` against the actual
...
class TestEachOfTheEightAlwaysBreakCharactersIsDetected(unittest.TestCase):
    ...
    split would -- the exact shape `gitq.has_splitlines_divergence` exists to
    ...
    def test_each_character_is_a_real_divergence_and_is_detected(self):
        for char in _EIGHT_ALWAYS_BREAK_CHARS:
            with self.subTest(char=hex(ord(char))):
                ...
                self.assertTrue(gitq.has_splitlines_divergence(text))
    ...
    def test_none_of_the_eight_false_positive_on_ordinary_text(self):
        ...
        self.assertFalse(gitq.has_splitlines_divergence(text))
...
class TestReadSnippetSourceThroughARealRepo(unittest.TestCase):
    ...
    a lone "\\r" only fails to reach `gitq.has_splitlines_divergence` when
